...
# Use python.exe for both paths. The GUI hides and detaches its console after Tk starts,
# while this launcher can still capture an early startup failure instead of losing it in pythonw.exe.
$preferConsoleMode = $true
...
try {
    ...
    try {
        ...
        if (-not $consoleCliMode) {
            $previousWindowedGui = $env:CODEX_RE6_ARC_WINDOWED_GUI
            $env:CODEX_RE6_ARC_WINDOWED_GUI = "1"
            try {
                ...
                if (-not $process.HasExited) {
                    Write-LaunchLogLine ("GuiPID: " + $process.Id)
                    Write-LaunchLogLine "ExitCode: 0 (GUI started)"
                ...
                if ($null -eq $previousWindowedGui) {
                    Remove-Item Env:CODEX_RE6_ARC_WINDOWED_GUI -ErrorAction SilentlyContinue
                }
                else {
                    $env:CODEX_RE6_ARC_WINDOWED_GUI = $previousWindowedGui
                }
            }
        }
        $process = Start-Process `
